...
# Get Projects
try:
    req = urllib.request.Request(f"{BASE_URL}/projects/", headers={'Authorization': f'Bearer {access_token}'})
    with urllib.request.urlopen(req) as res:
        print(f"Projects Status: {res.getcode()}")
        print(f"Projects Response: {res.read().decode('utf-8')}")
except HTTPError as e:
    print(f"Projects Failed: {e.code} - {e.read().decode('utf-8')}")
